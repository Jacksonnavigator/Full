...
function UtilityManagersPage() {
    ...
    async function fetchManagers() {
        try {
            const response = await fetch("/api/utility-managers");
            if (response.ok) {
    ...
    const availableUtilities = utilities.filter((u)=>{
        ...
        // Check if utility has no manager assigned
        if (!u.managerId) return true;
        // When editing, include the utility currently assigned to this manager
        if (editingManager && u.managerId === editingManager.id) return true;
        return false;
    });
    ...
    async function handleDelete() {
        if (deleteId) {
            try {
                const response = await fetch(`/api/utility-managers?id=${deleteId}`, {
                    method: "DELETE"
                });
                const data = await response.json();
                if (!response.ok) {
...
_s(UtilityManagersPage, "c3Vq53HYEXe8dRD2R+SLqgBiSrs=", false, function() {
    return [
...
//# sourceMappingURL=_89197103._.js.map