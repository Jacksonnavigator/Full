...
function DMAManagersPage() {
    ...
    const { dmas, utilities, fetchDMAs, fetchUtilities } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$store$2f$data$2d$store$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useDataStore"])();
    ...
    async function fetchManagers() {
        try {
            const response = await fetch("/api/dma-managers");
            if (response.ok) {
    ...
    const accessibleDMAManagers = isUtilityManager ? managers.filter((m)=>{
    ...
    async function handleDelete() {
        if (!deleteId) return;
        try {
            const response = await fetch(`/api/dma-managers?id=${deleteId}`, {
                method: "DELETE"
            });
            if (!response.ok) {
                const error = await response.json();
...
//# sourceMappingURL=_f5a9452d._.js.map