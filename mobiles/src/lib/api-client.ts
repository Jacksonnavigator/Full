...
class ApiClient {
  ...
  private async getAuthToken(): Promise<string | null> {
    try {
      const token = await AsyncStorage.getItem(CONFIG.JWT_TOKEN_STORAGE_KEY);
      console.log(`[API-Client] getAuthToken: token=${token ? 'present' : 'null'}, key=${CONFIG.JWT_TOKEN_STORAGE_KEY}`);
      return token;
    } catch (error) {
  ...
  private async prepareHeaders(
    ...
    if (!options.disableAuth) {
      ...
      if (token) {
        ...
        console.warn('[API-Client] No token available for Authorization header');
      }
    } else {
      console.log('[API-Client] disableAuth=true, skipping Authorization header');
    }
