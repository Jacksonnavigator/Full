...
export const LeaderPerformanceScreen: React.FC = () => {
  ...
  const awaitingReview = useMemo(
    () => teamTasks.filter((task) => isAwaitingLeaderActionTask(task)),
    [teamTasks]
  );
  ...
  const rejectedTasks = useMemo(
    () => teamTasks.filter((task) => isRejectedTaskStatus(task.status)),
    [teamTasks]
  );
  ...
  const closedTasks = useMemo(
    () => teamTasks.filter((task) => isClosedTaskStatus(task.status)),
    [teamTasks]
  );
  ...
  const engineerSnapshots = useMemo<EngineerSnapshot[]>(() => {
    ...
    teamTasks.forEach((task) => {
      ...
      if (isAwaitingLeaderActionTask(task)) {
        snapshot.awaitingReview += 1;
      }
      if (isClosedTaskStatus(task.status)) {
        snapshot.completed += 1;
      }
      if (isRejectedTaskStatus(task.status)) {
        snapshot.rejected += 1;
      }
    });
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top', 'left', 'right']}>
      ...
      >
        ...
        >
          <AppHeader
            title={getText(language, 'Performance', 'Utendaji')}
            subtitle={getText(
              language,
              `${currentUser?.team ?? 'Your team'} | action queue, team health, and field follow-up`,
              `${currentUser?.team ?? 'Timu yako'} | foleni ya kazi, afya ya timu, na ufuatiliaji wa uwanja`
            )}
          />
        </Animated.View>
