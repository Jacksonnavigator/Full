...
export const TaskDetailScreen: React.FC<Props> = ({ route, navigation }) => {
      ...
      try {
        ...
        setPipeNetworkMessage(
          preview
            ? getText(
                language,
                'Utility pipe network overlay loaded for this task area.',
                'Mstari wa mtandao wa bomba umepakiwa kwa eneo hili la kazi.'
              )
            : getText(
                language,
                'No previewable utility pipe network is uploaded for this utility yet.',
                'Hakuna mtandao wa bomba wawezao kuonekana kwa huduma hii bado.'
              )
        );
      } catch (error) {
        ...
        if (!isActive) return;
        setPipeNetworkPreview(null);
        setPipeNetworkMessage(getText(language, 'Pipe network could not be loaded right now.', 'Mtandao wa bomba haukuweza kupakiwa sasa hivi.'));
      } finally {
        if (isActive) {
  ...
  if (!task) {
    return (
      <View style={styles.centered}>
        <Text style={styles.errorText}>{getText(language, 'Task not found.', 'Kazi haikupatikana.')}</Text>
      </View>
    );
  }
  ...
  const formattedDistance =
    distanceMeters == null
      ? refreshingLocation
        ? getText(language, 'Refreshing...', 'Inarekebishwa...')
        : getText(language, 'Locating...', 'Inawekwa eneo...')
      : distanceMeters < 1000
      ? getText(language, `${distanceMeters.toFixed(0)} m away`, `${distanceMeters.toFixed(0)} m mbali`)
      : getText(language, `${(distanceMeters / 1000).toFixed(2)} km away`, `${(distanceMeters / 1000).toFixed(2)} km mbali`);
  ...
  const accuracyText =
    currentCoords?.accuracy != null
      ? getText(
          ...
          `Usahihi wa GPS takriban ${Math.round(currentCoords.accuracy)} m`
        )
      : getText(language, 'Waiting for GPS accuracy', 'Kusubiri usahihi wa GPS');

  const summaryCards = [
    {
      label: getText(language, 'Report type', 'Aina ya ripoti'),
      value: getText(language, task.reportType === 'non_leakage' ? 'Non-leakage' : 'Leakage', task.reportType === 'non_leakage' ? 'Sio uvujaji' : 'Uvujaji'),
      icon: 'layers-outline' as const,
      tone: '#0891b2',
    },
    {
      label: getText(language, 'Assigned team', 'Timu iliyoteuliwa'),
      value: task.assignedTeam || getText(language, 'Unassigned', 'Haijawekwa'),
      icon: 'people-outline' as const,
      tone: '#2563eb',
    },
    {
      label: getText(language, 'Team leader', 'Kiongozi wa timu'),
      value: task.teamLeader || getText(language, 'Not set', 'Haijawekwa'),
      icon: 'person-outline' as const,
      tone: '#7c3aed',
    },
    {
      label: getText(language, 'Distance', 'Umbali'),
  ...
  return (
    <>
      ...
      <AppHeader
        title={task ? task.title : getText(language, 'Task Details', 'Maelezo ya Kazi')}
        subtitle={task ? task.description : ''}
      />
      <View style={styles.stepsRow} accessibilityRole="summary">
          {[
            getText(language, 'Go to site', 'Nenda kwenye eneo'),
            getText(language, 'Start task', 'Anza kazi'),
            getText(language, 'Repair', 'Karabati'),
            getText(language, 'Submit', 'Tuma'),
          ].map((label, index) => {
      ...
      <View style={styles.sectionCard}>
        ...
        <View style={styles.mapContainer}>
          ...
          >
            ...
            />
            {currentCoords && (
              <Marker coordinate={currentCoords} title={getText(language, 'Your location', 'Mahali pako')} pinColor="blue" />
            )}
            <Circle
        ...
        <View style={styles.mapMetaStrip}>
          <View style={styles.mapMetaPill}>
            {loadingPipeNetwork ? (
              ...
              <Ionicons
                name={pipeNetworkPreview ? 'git-network-outline' : 'layers-outline'}
                size={14}
                color={pipeNetworkPreview ? '#0f766e' : colors.primary}
              />
            )}
          ...
          {pipeNetworkPreview && (
            <View style={styles.mapMetaPill}>
              <Ionicons name="remove-outline" size={14} color="#0f766e" />
              <Text style={styles.mapMetaText}>Green network lines show the uploaded utility pipe network</Text>
            </View>
          )}
        </View>
      </View>
            ...
            .map((entry, index) => (
              <View key={index} style={styles.timelineItem}>
                ...
                <View style={styles.timelineContent}>
                  ...
                  <Text style={styles.timelineTimestamp}>{formatTanzaniaDateTime(entry.timestamp)}</Text>
      ...
      <View style={styles.actionsCard}>
        ...
        >
          ...
          <Text style={styles.primaryButtonText}>
            {task.status === 'In Progress' || task.status === 'In Progress (Leader)'
              ? getText(language, 'Task In Progress', 'Kazi Inaendelea')
              : getText(language, 'Start Task', 'Anza Kazi')}
          </Text>
        </TouchableOpacity>
        ...
        >
          ...
          <Text style={styles.submitButtonText}>
            {role === 'Team Leader'
              ? getText(language, 'Resolve & Submit', 'Suluhisha & Tuma')
              : getText(language, 'Submit Repair Report', 'Tuma Ripoti ya Ukarabati')}
          </Text>
        </TouchableOpacity>
      </View>
      </ScrollView>
