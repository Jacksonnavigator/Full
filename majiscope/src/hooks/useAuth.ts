...
const normalizeRoleForStore = (role?: string | null, userType?: string | null) => {
  const normalizedUserType = userType?.trim().toLowerCase().replace(/\s+/g, '');
  if (normalizedUserType === 'dmamanager' || normalizedUserType === 'dma_manager') return 'DMA Manager';
...
const isMobileAppRoleAllowed = (role?: string | null, userType?: string | null) => {
  const normalizedRole = normalizeRoleForStore(role, userType);
  return (
    normalizedRole === 'Engineer' ||
    normalizedRole === 'Team Leader' ||
    normalizedRole === 'DMA Manager'
  );
};
...
const mapToStoreUser = (user: User) => ({
  ...
  role: normalizeRoleForStore(user.role, (user as any).user_type) as 'Engineer' | 'Team Leader' | 'DMA Manager',
  email: user.email,
  team: (user as any).team_name || (user as any).teamName || undefined,
    ...
    const initAuth = async () => {
      ...
      try {
        ...
        if (isAuth) {
          ...
          if (userData && !isMobileAppRoleAllowed(userData.role, (userData as any).user_type)) {
            await authManager.logout();
            setCurrentUser(null);
            useTaskStore.getState().logout();
            useDMAStore.getState().reset();
            setError('This mobile app is only available to engineers, team leaders, and DMA managers.');
          } else {
          ...
          if (userData && isMobileAppRoleAllowed(userData.role, (userData as any).user_type)) {
            if (normalizeRoleForStore(userData.role, (userData as any).user_type) === 'DMA Manager') {
              await useDMAStore.getState().refreshAllData();
            } else {
  ...
  const login = useCallback(async (credentials: LoginCredentials) => {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        if (!isMobileAppRoleAllowed(userToUse.role, (userToUse as any).user_type)) {
          await authManager.logout();
          useTaskStore.getState().logout();
          useDMAStore.getState().reset();
          const errorMessage = 'This mobile app is only available to engineers, team leaders, and DMA managers.';
          setCurrentUser(null);
        ...
        if (normalizeRoleForStore(userToUse.role, (userToUse as any).user_type) === 'DMA Manager') {
          await useDMAStore.getState().refreshAllData();
        } else {
          await useTaskStore.getState().fetchTasks();
        }
