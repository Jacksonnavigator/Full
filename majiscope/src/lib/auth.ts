...
export class AuthManager {
  ...
  async getUserData(): Promise<User | null> {
    try {
      ...
      const user: User & { user_type?: string | null } = JSON.parse(data);
      return {
        ...user,
        role: (this.normalizeRole(user.role, user.user_type) || 'Engineer') as User['role'],
      };
    } catch (error) {
   ...
   */
  private normalizeRole(role?: string | null, userType?: string | null): string | null {
    const normalizedUserType = userType?.trim().toLowerCase();
    if (normalizedUserType === 'dma_manager' || normalizedUserType === 'dma manager') {
      return 'DMA Manager';
    }
