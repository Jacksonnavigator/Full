...
export function formatTanzaniaWeekdayDate(value: DateInput, fallback = 'N/A') {
  const date = parseDate(value);
  if (!date) return fallback;
  return safeFormat(date, {
    weekday: 'long',
...
export function formatTanzaniaShortDateTime(value: DateInput, fallback = 'N/A') {
  const date = parseDate(value);
  if (!date) return fallback;
  return `${safeFormat(date, { month: 'short', day: 'numeric' })}, ${formatTanzaniaTime(date, fallback)}`;
}
